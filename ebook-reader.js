...
const PDFJS_SOURCES = [
  ...
  "https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.min.mjs"
];
...
async function importPdfJs() {
  ...
  for (const src of PDFJS_SOURCES) {
    try {
      ...
      return { lib, workerSrc: src.replace("pdf.min.mjs", "pdf.worker.min.mjs") };
    } catch (e) { lastErr = e; }
  }
  throw new Error("Could not load the PDF reader library from any source. Check your internet connection — if this keeps happening, your network may be blocking our content delivery sources.");
}

async function loadPdf(urlOrBytes) {
  const { lib: pdfjsLib, workerSrc } = await importPdfJs();
